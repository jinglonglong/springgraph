...
# springgraph (explore/node) to understand before editing, does it still Read? Like
# ab-sufficiency.sh but copies+indexes a FRESH target per run (the agent mutates
# it), so runs don't see each other's edits.
#
# WITH springgraph (pre-warmed) vs WITHOUT (empty MCP), N runs each. Reports
...
run(){ # label, withSpringgraph(0/1)
  ...
  for i in $(seq 1 "$RUNS"); do
    ...
    if [ "$wcg" = "1" ]; then
      printf '{"mcpServers":{"springgraph":{"command":"env","args":["SPRINGGRAPH_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$tgt" > "$cfg"
      prewarm "$tgt"
...
echo "== WITH springgraph =="; run with 1
