...
Springgraph collects a small set of **anonymous usage statistics** — which commands and
...
```bash
springgraph telemetry off        # stores your choice (and deletes any unsent data)
```

```bash
export SPRINGGRAPH_TELEMETRY=0   # per-shell / per-CI override
export DO_NOT_TRACK=1          # the cross-tool standard — always honored
```

`springgraph telemetry status` shows the current state, what decided it, and your machine ID.
The interactive installer (`springgraph install`) asks up front with a visible default-on
...
Off means off: when disabled, Springgraph records nothing, opens no connection to the
...
- **`install`** — when `springgraph install` configures agents: which agents
...
- **`usage_rollup`** — one line per day per tool: the tool or CLI command **name** (e.g.
  `springgraph_explore`, `init`), how many times it ran, how many errored, and — for MCP
  tools — the connecting agent's name and version from the MCP handshake (e.g.
  `Claude Code 2.1`).
- **`uninstall`** — when `springgraph uninstall`/`uninit` runs: which agents were removed.
...
- **No fingerprinting.** The machine ID is a random UUID stored in
  `~/.springgraph/telemetry.json` — delete that file (or run `springgraph telemetry off`,
...
Events POST to `telemetry.getspringgraph.com` — a first-party endpoint whose complete
