 ...
 * see watch-policy.ts), the Springgraph index would otherwise go stale until
 * the user runs `springgraph sync` by hand. As an opt-in alternative, we can
 ...
 *
 * The hooks run `springgraph sync` in the background so they never block git,
 * and are guarded by `command -v springgraph` so they no-op cleanly when the
...
const MARKER_BEGIN = '# >>> springgraph sync hook >>>';
const MARKER_END = '# <<< springgraph sync hook <<<';
...
function markerBlock(): string {
  return [
    MARKER_BEGIN,
    '# Keeps the Springgraph index fresh while the live file watcher is off',
    '# (e.g. WSL2 /mnt drives). Runs in the background so it never blocks git.',
    '# Managed by springgraph; remove with `springgraph uninit` or delete this block.',
    'if command -v springgraph >/dev/null 2>&1; then',
    '  ( springgraph sync >/dev/null 2>&1 & ) >/dev/null 2>&1',
    'fi',
    MARKER_END,
  ].join('\n');
}
...
/**
 * Install (or update) the Springgraph sync hooks in a git repository.
...
/**
 * Remove the Springgraph sync hooks. Strips only our marker block; deletes the
...
/** Whether any Springgraph sync hook is currently installed. */
