#!/usr/bin/env bash
# With/without A/B (and optional interactive) eval for a springgraph version on a
# repo. Springgraph is the ONLY variable: both arms launch claude with
# --strict-mcp-config — with = springgraph-only MCP (pointed at $CG_BIN),
...
# Env:   CG_BIN          springgraph binary (default: command -v springgraph)
...
MODE="${3:-headless}"
CG_BIN="${CG_BIN:-$(command -v springgraph)}"
OUT="${AGENT_EVAL_OUT:-/tmp/agent-eval}"
HARNESS="$(cd "$(dirname "$0")" && pwd)"
mkdir -p "$OUT"

[ -n "$CG_BIN" ] || { echo "no springgraph binary on PATH (set CG_BIN)"; exit 1; }
[ -d "$REPO/.springgraph" ] || { echo "no .springgraph index at $REPO — index it first"; exit 1; }
...
cat > "$OUT/mcp-springgraph.json" <<JSON
{"mcpServers":{"springgraph":{"command":"$CG_BIN","args":["serve","--mcp","--path","$REPO"]}}}
JSON
echo '{"mcpServers":{}}' > "$OUT/mcp-empty.json"

echo "###### springgraph: $CG_BIN"
echo "###### repo:      $REPO"
echo "###### question:  $Q"
echo
...
if [ "$MODE" = headless ] || [ "$MODE" = all ]; then
  headless "headless-with"    "$OUT/mcp-springgraph.json"
  headless "headless-without" "$OUT/mcp-empty.json"
fi

if [ "$MODE" = tmux ] || [ "$MODE" = all ]; then
  echo "############################## INTERACTIVE [with] ##############################"
  CLAUDE_EXTRA_ARGS="--model ${MODEL:-sonnet} --effort ${EFFORT:-high} --strict-mcp-config --mcp-config $OUT/mcp-springgraph.json" \
