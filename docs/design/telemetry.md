...
`springgraph telemetry` CLI, MCP + installer wiring, `TELEMETRY.md`. Pending: Worker deploy
+ DNS, release.
Scope: public `springgraph` engine (CLI + MCP server + installer)

Springgraph is a local-first tool whose whole pitch is "your code never leaves your machine."
...
- How many machines actively use springgraph (daily/weekly), and how does that change?
...
- No telemetry from the `springgraph-pro` fork (see "springgraph-pro rule" below).
...
4. **First-party endpoint.** Clients only ever talk to `telemetry.getspringgraph.com`. The URL
...
| `springgraph_version` | `0.9.12` | from package.json |
...
- **`usage_rollup`** — the workhorse. One event per `(day, kind, name)` per machine,
  ...
  (e.g. `springgraph_explore`, `affected`), `count`, `error_count`, and for MCP:
...
2. `SPRINGGRAPH_TELEMETRY=0|1` → forced off/on for that process
3. Global config `~/.springgraph/telemetry.json` → stored user choice
...
- **Headless paths** (`npx springgraph init`, MCP server — no TTY, never prompt): right
  ...
  **stderr** and record `first_run_notice_shown`:
  `springgraph collects anonymous usage stats (no code or paths) — "springgraph telemetry off" or SPRINGGRAPH_TELEMETRY=0 disables. Details: TELEMETRY.md`
- **CLI:** `springgraph telemetry status|on|off` (status prints the machine ID, current
  state, and what decided it). Deleting `~/.springgraph/telemetry.json` resets everything,
  including the machine ID.

`~/.springgraph/telemetry.json`:
...
(`~/.springgraph/` is new — today nothing global exists. Coexists by filename if a user ever
indexes `$HOME` itself, since per-project data lives in `<project>/.springgraph/` with fixed
other filenames.)
...
- **Buffer** — counters persist (debounced, async) to `~/.springgraph/telemetry-queue.jsonl`.
...
- **Flush** — many CLI actions end via `process.exit()`, where `beforeExit` never fires
  ...
  `https://telemetry.getspringgraph.com/v1/events` with `AbortSignal.timeout(1500)`,
  ...
  sender's claim merges back after an hour). `SPRINGGRAPH_TELEMETRY_DEBUG=1` echoes
...
`telemetry.getspringgraph.com` → small Worker living at `telemetry-worker/` in this repo —
...
## springgraph-pro rule (do not lose this in upstream merges)

The private `springgraph-pro` fork ships inside customer containers whose guarantee is
...
container sets `SPRINGGRAPH_TELEMETRY=0` as belt-and-braces. This rule lives in the fork's
...
2. Worker + DNS live first (so the first shipping client never 404s), PostHog dashboards:
   ...
   languages indexed.
3. Client module + config + `springgraph telemetry` subcommand + MCP `clientInfo` plumbing.
