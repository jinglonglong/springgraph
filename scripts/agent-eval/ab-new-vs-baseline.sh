#!/usr/bin/env bash
# A/B a springgraph retrieval/steering change: the NEW build (current HEAD) vs a
# BASELINE build (a git ref) — BOTH with springgraph attached — on the same
# implementation task, measuring how many Read vs springgraph calls the agent
...
# session): each arm PRE-WARMS a persistent springgraph daemon for its target so
# claude connects to an already-bound, index-loaded daemon instantly — before
# the agent's first turn — and SKIPS springgraph's startup re-exec via
# SPRINGGRAPH_WASM_RELAUNCHED=1. Without this, on a multi-step task the agent
# dives into Read/grep before springgraph finishes its ~2-3s startup (worse under
# the CPU contention of a nested run) and runs with NO springgraph.
#
# Gotcha: claude's `system/init` snapshot can read status:"pending" / 0 tools
# even when the server then connects fine — judge by ACTUAL springgraph usage in
# parse-run.mjs's "by type", not the init line.
#
# Usage: ab-new-vs-baseline.sh <indexed-repo> "<task>" [baseline-ref]
#   <indexed-repo>  a repo with a .springgraph index (copied per arm)
...
BIN="$ENGINE/dist/bin/springgraph.js"
OUT="${AGENT_EVAL_OUT:-/tmp/ab-new-vs-baseline}"
PARSE="$ENGINE/scripts/agent-eval/parse-run.mjs"

command -v claude >/dev/null || { echo "claude CLI not on PATH"; exit 1; }
[ -d "$TARGET/.springgraph" ] || { echo "target not indexed: run 'springgraph init $TARGET' first"; exit 1; }
...
rsync -a --exclude node_modules --exclude .git --exclude dist --exclude .springgraph "$TARGET/" "$OUT/t-new/"
...
prewarm() { # target — spawn a persistent daemon (current $BIN) and wait for its socket
  ...
  SPRINGGRAPH_DAEMON_IDLE_TIMEOUT_MS=1800000 node "$BIN" serve --mcp --path "$1" </dev/null >/dev/null 2>&1 &
  node -e 'const fs=require("fs");let n=0;const t=setInterval(()=>{if(fs.existsSync(process.argv[1]+"/.springgraph/daemon.sock")){clearInterval(t);process.exit(0)}if(n++>150){clearInterval(t);process.exit(1)}},100)' "$1" \
    && echo "  daemon warm: $1" || echo "  WARN: daemon never bound for $1 (arm may run without springgraph)"
}
...
run_arm() { # label, target-copy
  ...
  printf '{"mcpServers":{"springgraph":{"command":"env","args":["SPRINGGRAPH_WASM_RELAUNCHED=1","node","%s","serve","--mcp","--path","%s"]}}}' "$BIN" "$tgt" > "$c"
  prewarm "$tgt"
  echo "############## ARM [$label] ##############"
...
echo "###### (especially Read vs mcp__springgraph__*). Full logs in: $OUT"
