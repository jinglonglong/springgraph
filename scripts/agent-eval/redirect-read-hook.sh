...
# onto springgraph_node during implementation, not just for Q&A).
#
# When the agent Reads a SOURCE file, deny it and steer to springgraph_node's
...
# A redirect to a file springgraph hasn't indexed SELF-CORRECTS: the file-view
...
# Eval artifact only. The production version is an indexed-aware `springgraph`
