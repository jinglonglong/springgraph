 ...
 * SPRINGGRAPH_DOWNLOAD_BASE — no real network, no published release needed. The
...
const asset = `springgraph-${target}.tar.gz`;
...
// A temp dir standing in for the installed @colbymchenry/springgraph main package.
function makePkg(version = '9.9.9-test'): string {
  const dir = mkTmp('pkg');
  fs.copyFileSync(SHIM_SRC, path.join(dir, 'npm-shim.js'));
  fs.writeFileSync(path.join(dir, 'package.json'),
    JSON.stringify({ name: '@colbymchenry/springgraph', version }) + '\n');
  return dir;
}
...
function writeLauncher(binDir: string): void {
  ...
  const p = path.join(binDir, 'springgraph');
...
describe.skipIf(isWindows)('npm-shim launcher', () => {
  it('runs the installed optional-dependency bundle without any download', async () => {
    const pkg = makePkg();
    const platformPkg = path.join(pkg, 'node_modules', '@colbymchenry', `springgraph-${target}`);
    writeLauncher(path.join(platformPkg, 'bin'));
    fs.writeFileSync(path.join(platformPkg, 'package.json'),
      JSON.stringify({ name: `@colbymchenry/springgraph-${target}`, version: '9.9.9-test' }) + '\n');
    const cache = mkTmp('cache');
    const r = await runShim(pkg, ['--probe-abc'], { SPRINGGRAPH_INSTALL_DIR: cache });
  ...
  it('uses an already-cached bundle even when downloads are disabled', async () => {
    ...
    const r = await runShim(pkg, ['--probe-xyz'], {
      SPRINGGRAPH_INSTALL_DIR: cache,
      SPRINGGRAPH_NO_DOWNLOAD: '1',
    });
  ...
  it('prints actionable guidance and exits 1 when disabled with no bundle', async () => {
    ...
    const r = await runShim(pkg, ['--version'], {
      SPRINGGRAPH_INSTALL_DIR: mkTmp('cache'),
      SPRINGGRAPH_NO_DOWNLOAD: '1',
    });

    expect(r.status).toBe(1);
    expect(r.stderr).toContain(`no prebuilt bundle for ${target}`);
    expect(r.stderr).toContain(`@colbymchenry/springgraph-${target}`);
...
describe.skipIf(!CAN_NET)('npm-shim download fallback (local HTTPS)', () => {
  ...
  beforeAll(async () => {
    ...
    // Build a fake bundle archive (springgraph-<target>/bin/springgraph), like a real release asset.
    const work = mkTmp('fixture');
    writeLauncher(path.join(work, `springgraph-${target}`, 'bin'));
    const archive = path.join(work, asset);
    execSync(`tar -czf ${JSON.stringify(archive)} -C ${JSON.stringify(work)} springgraph-${target}`);
  ...
  function netEnv(cache: string): Record<string, string> {
    return {
      SPRINGGRAPH_INSTALL_DIR: cache,
      SPRINGGRAPH_DOWNLOAD_BASE: `https://127.0.0.1:${port}`,
  ...
  it('downloads, verifies the checksum, extracts, and execs the bundle', async () => {
    ...
    expect(fs.existsSync(path.join(cache, 'bundles', `${target}-5.0.0-net`, 'bin', 'springgraph'))).toBe(true);
  }, 20000);
