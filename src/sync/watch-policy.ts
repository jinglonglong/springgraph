 ...
 *  1. `SPRINGGRAPH_NO_WATCH=1`    → off  (explicit opt-out always wins)
 *  2. `SPRINGGRAPH_FORCE_WATCH=1` → on   (overrides auto-detection)
...
export function watchDisabledReason(projectRoot: string, probe: WatchProbe = {}): string | null {
  ...
  if (env.SPRINGGRAPH_NO_WATCH === '1') {
    return 'SPRINGGRAPH_NO_WATCH=1 is set';
  }
  if (env.SPRINGGRAPH_FORCE_WATCH === '1') {
    return null;
  }
