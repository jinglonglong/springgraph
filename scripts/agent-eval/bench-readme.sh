#!/usr/bin/env bash
# Re-run the README "Benchmark Results" A/B (with vs without springgraph) on the
...
# $CORPUS (default /tmp/springgraph-corpus) by the build under test.
set -uo pipefail
H="$(cd "$(dirname "$0")" && pwd)"
C="${CORPUS:-/tmp/springgraph-corpus}"
RUNS="${RUNS:-4}"
ROWS=(
