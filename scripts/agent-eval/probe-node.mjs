#!/usr/bin/env node
// Probe springgraph_node (with trail) against an index using the built dist.
// Usage: node probe-node.mjs <repo-with-.springgraph> <symbol> [code]
...
const res = await h.execute('springgraph_node', { symbol, includeCode: code === 'code' });
