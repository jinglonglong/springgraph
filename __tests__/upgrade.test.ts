...
describe('detectInstallMethod', () => {
  ...
  it('detects a unix bundle and derives the install dir from the versions/ layout', () => {
    const root = '/home/u/.springgraph/versions/v0.9.9';
    const filename = `${root}/lib/dist/bin/springgraph.js`;
    const present = new Set([`${root}/node`, `${root}/bin/springgraph`, '/home/u/.springgraph']);
    ...
    expect(m).toEqual({
      ...
      os: 'unix',
      bundleRoot: root,
      installDir: '/home/u/.springgraph',
    });
  });

  it('detects a windows bundle and derives the install dir from current\\', () => {
    const root = 'C:/Users/u/AppData/Local/springgraph/current';
    const filename = `${root}/lib/dist/bin/springgraph.js`;
    const present = new Set([`${root}/node.exe`, `${root}/bin/springgraph.cmd`]);
    const m = detectInstallMethod({
      filename,
      platform: 'win32',
    ...
    expect(m.installDir?.replace(/\\/g, '/')).toBe('C:/Users/u/AppData/Local/springgraph');
  });
  ...
  it('detects a source checkout via sibling package.json + .git', () => {
    const repo = '/home/u/dev/springgraph';
    const filename = `${repo}/dist/bin/springgraph.js`;
  ...
  it('returns unknown for an unrecognized layout', () => {
    const m = detectInstallMethod({
      filename: '/opt/weird/place/springgraph.js',
      platform: 'linux',
      cwd: '/tmp',
      exists: () => false,
    });
...
describe('deriveInstallDir', () => {
  it('unix: returns the dir above versions/', () => {
    expect(deriveInstallDir('/a/b/.springgraph/versions/v1.2.3', 'unix', () => true)).toBe('/a/b/.springgraph');
  });
  ...
  it('windows: returns the parent of current\\', () => {
    expect(deriveInstallDir('C:/x/springgraph/current', 'windows', () => true)?.replace(/\\/g, '/')).toBe('C:/x/springgraph');
  });
  it('windows: null when basename is not current', () => {
    expect(deriveInstallDir('C:/x/springgraph/v1', 'windows', () => true)).toBeNull();
  });
});
...
describe('version helpers', () => {
  ...
  it('reindexAdvisory mentions the refresh commands', () => {
    ...
    expect(a).toContain('springgraph sync');
    expect(a).toContain('springgraph index -f');
  });
  ...
  it('buildWindowsUpgradeScript targets the right asset per arch and renames-not-deletes the exe', () => {
    ...
    expect(arm).toContain('releases/download/v1.2.3/springgraph-win32-arm64.zip');
    ...
    expect(x64).toContain('springgraph-win32-x64.zip');
  });
});
...
describe('runUpgrade', () => {
  ...
  it('unix bundle: runs the installer via sh with the derived install dir', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.springgraph/versions/v0.9.8', installDir: '/h/.springgraph' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
    ...
    expect(calls.runs[0].env?.SPRINGGRAPH_INSTALL_DIR).toBe('/h/.springgraph');
    expect(calls.logs.join('\n')).toMatch(/springgraph sync/); // re-index advisory printed
  });

  it('unix bundle: falls back to wget, and errors when neither downloader exists', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.springgraph/versions/v0.9.8', installDir: null },
      currentVersion: '0.9.8',
      hasCommand: () => false,
    });
    const code = await runUpgrade({}, deps);
  ...
  it('windows bundle: runs a synchronous in-place (rename + extract) powershell upgrade', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'windows', bundleRoot: 'C:/x/springgraph/current', installDir: 'C:/x/springgraph' },
      currentVersion: '0.9.8',
      platform: 'win32',
    });
    const code = await runUpgrade({}, deps);
    ...
    expect(decoded).toContain('releases/download/v0.9.9/springgraph-win32-');
  ...
  it('source: tells the user to git pull, runs nothing', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'source', root: '/dev/springgraph' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
...
describe('index extraction-version stamp / isIndexStale', () => {
  ...
  it('stamps the current extraction version on full index and is not stale', async () => {
    ...
    const cg = await Springgraph.init(dir, { index: false });
