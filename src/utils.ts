 ...
 * @example
 * ```typescript
 * import { Mutex, processInBatches, MemoryMonitor, validatePathWithinRoot } from 'springgraph';
 *
 ...
 * API key, JDBC URL with embedded creds). Springgraph must surface the KEY only
 ...
 * (springgraph_node `includeCode`, springgraph_explore source) go through here, so
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      try {
        ...
        if (lockAge < FileLock.STALE_TIMEOUT_MS && !isNaN(pid) && this.isProcessAlive(pid)) {
          throw new Error(
            `Springgraph database is locked by another process (PID ${pid}). ` +
            `If this is stale, run 'springgraph unlock' or delete ${this.lockPath}`
          );
        }
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        throw new Error(
          'Springgraph database is locked by another process. ' +
          `If this is stale, run 'springgraph unlock' or delete ${this.lockPath}`
        );
      }
      throw err;
