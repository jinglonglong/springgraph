...
**Question:** Does answering a "how does X work?" question *directly* with Springgraph in the
...
**Short answer:** No. With Springgraph, main-session context is roughly **scale-invariant (~50k)**
...
- **Arms:** `WITH` = Springgraph in the MCP config; `WITHOUT` = empty MCP config (`--strict-mcp-config`).
...
| metric | WITH springgraph | WITHOUT |
|---|---|---|
...
| springgraph calls | ~8 (search + explore×2–3 + context) | 0 |
| Reads (main+sub) | 0 / 1 / 0 | 6 / 26 / 19 |
| billable tokens | ~126k | ~176k |

## Findings

**Main-session context is scale-invariant with Springgraph.** With springgraph, main-session
context was **~47k on VS Code — essentially identical to Excalidraw's ~50k**, despite a 16×
bigger repo. It didn't balloon. Reason: springgraph's `explore` payload is **budget-capped** and
retrieval is **targeted** — answering one question pulls in the relevant *flow/area*, not more
just because the repo is huge. So springgraph makes main-session context roughly scale-invariant
...
Claude Code avoids that **without** springgraph by delegating to an Explore agent (29–31k main),
but at the cost of **17–26 reads** and ~28% more tokens. Springgraph keeps main lean a *better*
way: a capped, targeted payload — no delegation, **0 reads**.

**On "the Explore agents use springgraph."** I couldn't reproduce it: across **6/6**
with-springgraph runs (both repos), Claude Code **never delegated** — it answered directly every
time. The Explore-agent path only appeared in the `without` arm (using grep/read, since springgraph
wasn't in that config). So with the current instructions + springgraph present, Claude Code stays
in the main session — the lean-main-via-Explore-agent best case simply isn't what happens;
lean-main-via-capped-springgraph is, and it's cheaper.

## Verdict

**"Answer directly with springgraph" wins for Claude Code too — at every scale.** No per-agent
...
directly). This conclusion drove updating the README's `## Springgraph` example block, which
previously told agents to "NEVER call `springgraph_explore` directly / ALWAYS spawn an Explore
agent" — i.e., it steered Claude Code toward the *worse* (17–26 read, ~28%-more-token) path.

**Caveat / future work (not a blocker):** an Explore agent that *itself uses springgraph* could in
