/**
 * `springgraph affected` input-path normalization (#825).
 *
...
describe('springgraph affected — input path normalization (#825)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'springgraph-affected-paths-'));
