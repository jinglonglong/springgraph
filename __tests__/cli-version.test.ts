/**
 * Tests for the `springgraph version` affordances.
 *
...
describe('springgraph version affordances', () => {
  for (const spelling of ['version', '-v', '-version', '--version', '-V']) {
    it(`\`springgraph ${spelling}\` prints exactly the package version`, () => {
  ...
  it('`springgraph help` prints usage and the command list', () => {
    const out = run(['help']);
    expect(out).toContain('Usage: springgraph');
  ...
  it('a trailing `-v` is still the subcommand\'s --verbose, not the version intercept', () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'springgraph-version-test-'));
    let combined = '';
    try {
