...
describe('buildNode25BlockBanner', () => {
  ...
  it('documents the SPRINGGRAPH_ALLOW_UNSAFE_NODE override', () => {
    const banner = buildNode25BlockBanner('25.7.0');
    expect(banner).toContain('SPRINGGRAPH_ALLOW_UNSAFE_NODE=1');
  });

  it('links to issue #81 for the root-cause writeup', () => {
    expect(buildNode25BlockBanner('25.7.0')).toContain(
      'github.com/colbymchenry/springgraph/issues/81'
    );
  });
});
...
describe('buildNodeTooOldBanner', () => {
  ...
  it('documents the SPRINGGRAPH_ALLOW_UNSAFE_NODE override', () => {
    expect(buildNodeTooOldBanner('18.0.0')).toContain('SPRINGGRAPH_ALLOW_UNSAFE_NODE=1');
  });
});
