 ...
 * Opt out with `SPRINGGRAPH_NO_WATCHDOG=1`; tune with `SPRINGGRAPH_WATCHDOG_TIMEOUT_MS`.
 */
...
function debug(msg: string): void {
  if (process.env.SPRINGGRAPH_MCP_DEBUG) {
    try { fs.writeSync(2, `[Springgraph watchdog] ${msg}\n`); } catch { /* ignore */ }
  }
}
...
const MSG = Buffer.from('[Springgraph] Main thread unresponsive for ~' + secs + 's — killing the wedged process so a fresh one can start (#850). Disable with SPRINGGRAPH_NO_WATCHDOG=1.\\n');
function kill() {
...
export function installMainThreadWatchdog(): WatchdogHandle | null {
  if (isEnvTruthy(process.env.SPRINGGRAPH_NO_WATCHDOG)) return null;

  const timeoutMs = parseWatchdogTimeoutMs(process.env.SPRINGGRAPH_WATCHDOG_TIMEOUT_MS);
