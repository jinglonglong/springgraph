#!/usr/bin/env bash
# Headless Claude Code run against a repo with springgraph MCP, capturing the
...
# Env: AGENT_EVAL_OUT (default /tmp/agent-eval), CG_BIN (springgraph dist binary)
set -uo pipefail

REPO="$1"; LABEL="$2"; PROMPT="$3"
CG_BIN="${CG_BIN:-$(command -v springgraph || echo /usr/local/bin/springgraph)}"
